...
export default function DraftEditorPage({ params }: EditorPageProps) { // Changed type annotation to EditorPageProps to maintain syntactic correctness
    ...
    useEffect(() => {
        const timeoutId = setTimeout(async () => {
            ...
            if (content !== (draft as any).editedText || title !== (draft as any).title || clientName !== (draft as any).clientName || status !== (draft as any).status) {
                setSaving(true);
                try {
                    ...
                    setDraft(prev => prev ? { ...prev, editedText: content, title: title, clientName: clientName, status: status } as any : prev);
                } catch (e) {
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            console.log("🔵 Updating drafts document...");
            await updateDoc(doc(db, 'drafts', draftId), {
                editedText: content,
                title: title,
                status: 'approved',
        ...
        return (
            <AuthGuard>
                <div className="min-h-screen bg-slate-50 p-8 flex flex-col items-center justify-center">
                    ...
                    <button onClick={() => router.push('/')} className="text-slate-600 underline hover:text-slate-900">חזרה לתיבת הידע</button>
                </div>
            </AuthGuard>
        );
    }
            ...
            <div className="min-h-screen bg-slate-50 flex flex-col overflow-hidden">
                ...
                <header className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                        <div className="flex items-center space-x-2 space-x-reverse">
                            <button
                                onClick={() => router.push('/')}
                                className="p-2 -me-2 text-slate-400 hover:text-slate-700 rounded-full hover:bg-slate-100 transition-colors"
                            >
                                <ArrowLeft className="w-5 h-5 rotate-180" />
                            </button>
                            <h1 className="text-lg font-bold text-slate-900 hidden sm:block">עורך טיוטה</h1>
                        </div>

                        <div className="flex items-center space-x-4 space-x-reverse">
                            <span className="text-sm flex items-center text-slate-500 font-medium tracking-wide w-28 justify-end">
                                {saving ? (
                                    <span className="flex items-center text-amber-600">
                                        <Save className="w-4 h-4 me-1.5 animate-bounce" /> שומר...
                                    </span>
                                ) : (
                                    <span className="flex items-center text-emerald-600">
                                        <CheckCircle className="w-4 h-4 me-1.5" /> נשמר
                                    </span>
                                )}
                            </span>
                            <button
                ...
                <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 overflow-hidden h-[calc(100vh-4rem)]">
                    ...
                    <div className="flex flex-col space-y-3 h-full overflow-hidden">
                        <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest pe-1">מקור גולמי</h2>
                    ...
                    <div className="flex flex-col space-y-3 h-full overflow-hidden">
                        <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest pe-1">ידע מובנה ומסודר</h2>
                        <div className="bg-white rounded-xl shadow-md border border-slate-200 flex-1 flex flex-col overflow-hidden focus-within:ring-1 focus-within:ring-slate-300 transition-shadow">
                            ...
                            <div className="p-4 sm:p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col space-y-4">
                                <input
                                    type="text"
                                    placeholder="תן כותרת למסמך..."
                                    value={title}
                                ...
                                <div className="flex items-center space-x-4 space-x-reverse text-sm">
                                    <select 
                                        ...
                                        className="bg-white border border-slate-200 text-slate-700 rounded-md px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-slate-900 shadow-sm"
                                    >
                                        <option value="">-- שיוך לקוח (לא חובה) --</option>
                                        <option value="T&M">T&M</option>
                                        <option value="Microsoft Israel">Microsoft Israel</option>
                                        <option value="Bank Leumi">Bank Leumi</option>
                                        <option value="Internal">פנימי (PavelKnox)</option>
                                    </select>
                                    ...
                                    <select 
                                        ...
                                        className="bg-white border border-slate-200 text-slate-700 rounded-md px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-slate-900 shadow-sm"
                                    >
                                        <option value="pending">מעורב לטיפול (Pending)</option>
                                        <option value="in_progress">בטיפול (In Progress)</option>
                                        <option value="approved">מוכן (Ready)</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <textarea
                                ...
                                placeholder="כתוב, תרגם או ארגן את המידע המקצועי כאן... עיצוב Markdown נתמך."
                                value={content}
