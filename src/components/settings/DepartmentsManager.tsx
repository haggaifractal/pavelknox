            ...
            <div className="p-6">
                <form onSubmit={handleCreate} className="flex flex-col sm:flex-row gap-4 items-end mb-8">
                    ...
                    <div className="flex-1 w-full relative">
                        <label className="block text-xs font-semibold mb-2 text-slate-500">{t('settings.departmentDescLabel')}</label>
                        <input
                            type="text"
                            value={newDesc}
                            onChange={(e) => setNewDesc(e.target.value)}
                            className="w-full px-4 py-2 bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                            placeholder={t('settings.departmentDescPlaceholder')}
                        />
                    </div>
                    <button
                        type="submit"
                    ...
                    <div className="border border-slate-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                        <table className="w-full text-sm text-left rtl:text-right">
                            <thead className="bg-slate-50 dark:bg-zinc-950/50 text-slate-600 dark:text-zinc-400 text-xs font-semibold border-b border-slate-200 dark:border-zinc-800">
                                <tr>
                                    <th className="px-6 py-3">{t('settings.departmentNameLabel')}</th>
                                    <th className="px-6 py-3">{t('settings.departmentDescLabel')}</th>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
                                {departments.map(dep => (
                                    <tr key={dep.id} className="hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors">
                                        <td className="px-6 py-4 font-medium text-slate-800 dark:text-zinc-200">{dep.name}</td>
                                        <td className="px-6 py-4 text-slate-500 dark:text-zinc-400">{dep.description || '-'}</td>
